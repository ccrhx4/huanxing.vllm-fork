...
# interleaved SwiGLU MoE fusion kernel. Default model is Qwen3-30B-A3B,
# TP=4; override MODEL / RESULT_ROOT (and TP_SIZE if needed) to target a
# different model, e.g.:
...
#
# For each configuration (baseline: VLLM_XPU_FUSED_MOE_INTERLEAVED unset,
# fusion: VLLM_XPU_FUSED_MOE_INTERLEAVED=1) this script:
...
#   4. Stops the server.
#
# Usage: benchmarks/moe_fusion/run_moe_fusion_bench_compare.sh [baseline|fusion|both]
# Default: both.
set -o pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

MODEL="${MODEL:-/hf_models/Qwen3-30B-A3B}"
TP_SIZE="${TP_SIZE:-4}"
PORT="${PORT:-8115}"
RESULT_ROOT="${RESULT_ROOT:-/work/bench_results_moe_interleaved_fusion}"
...
run_one() {
  ...
  echo "=============================================="
  echo "Running config: $tag (VLLM_XPU_FUSED_MOE_INTERLEAVED=$fuse_flag)"
  echo "=============================================="

  source /opt/intel/oneapi/setvars.sh --force > /tmp/setvars_bench.log 2>&1

  if [ "$fuse_flag" = "1" ]; then
    export VLLM_XPU_FUSED_MOE_INTERLEAVED=1
  else
    unset VLLM_XPU_FUSED_MOE_INTERLEAVED
  fi
