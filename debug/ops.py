...
def flat_pa(query, key_cache, value_cache, block_list, block_mapping,
    ...
    query = batch2block(query_scaled, block_mapping).unsqueeze(-2)
    ...
    block_mapping = block_mapping.to(torch.bfloat16)
...
def prompt_attention(
    ...
    if key.isnan().any() or value.isnan().any() or query.isnan().any():
        print("key/value/query is nan.")
    ...
    if attn_bias is not None or HPUFusedSDPA is None:
        ...
        query_scaled = query * scale
        attn_weights = matmul_qk_op(query_scaled, key.transpose(-1, -2))
        if attn_bias is not None:
